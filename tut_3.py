...
class TUT_3(ModuleBase):
    ...
    def process_input(self, datablock):
        ...
        while self.eeg_channels.shape[1] > self.chunk:
            ...
            global tx112358tx
            tx112358tx = time.clock()
            print 'tx: '+str(tx112358tx)
            ...
            #print 'bufcopy' +  str(bufcopy.size)
...
class _OnlineCfgPane(Qt.QFrame):
    ...
    def __init__(self , *args):
        ...
        self.comboBoxChunk.addItem(Qt.QString("500"))
...
class _FFT_Plot(Qwt.QwtPlot):
    ...
    def calculate(self, channel_data):
        ...
        #[ar, var, reflec] = spectrum.aryule(channel_data, 20, norm= 'biased') #20 is index_min of AIC
        #psd = spectrum.arma2psd(ar)
        #freqvals = np.linspace(0,500/2,len(psd)/2) #500 is frequency
        b,a = signal.butter(1, [7/(0.5*500), 12/(0.5*500)], btype='band') #7 and 10 are min and max freq
        cleanlast1second = signal.filtfilt(b, a, channel_data) #zero phase band pass
        
        predictionoverlaplength = 0.2 #how far forward you want to predict, in percent
        frames = round(predictionoverlaplength*len(cleanlast1second)) #calculate that in frames
        ARmodel= sm.tsa.AR(cleanlast1second[frames:(-1*frames)]) #get autoregressive model
        ARmodelfit = ARmodel.fit(ic='aic') #fit the model
        ARmodelpredict = ARmodel.predict(n = 2*frames, start = len(cleanlast1second)-2*frames, method = 'dynamic')
        ...
        inst_freq = np.diff(inst_phase)/(2*np.pi)*500 #get the frequencies at each timepoint, 500 is frequency
        t1=time.clock()
        timelag = t1 - tx112358tx
        if timelag <= 0.1:
            phaseguess = np.degrees(inst_phase[500+round(timelag*500)]) #instant phase at current time point
            freqguess = inst_freq[500+round(timelag*500)] #instant frequency at current time point

            #print 'cleanlast1second: ' + str(cleanlast1second.size)
            #print 'inst_freq: ' + str(inst_freq.size)
            ...
            t3=time.clock()
            timedelayerror= t3 - t1 - timedelay
            serial.Serial('COM4', 38400).write([1])
        print 'delivered. timelag: ' + str(timelag) + ' timedelay: ' + str(timedelay) + ' timedelayerror: ' + str(timedelayerror)
